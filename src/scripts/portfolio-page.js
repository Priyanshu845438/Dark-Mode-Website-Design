...
class PortfolioPage {
    ...
    async loadComponents() {
        try {
            ...
            if (headerContainer && window.HeaderComponent) {
                ...
                header.setActivePath('/company/portfolio');
            }
    ...
    setupEventListeners() {
        ...
        statCards.forEach(card => {
            card.addEventListener('mouseenter', () => {
                this.animateStatNumber(card, true);
            });
            
            card.addEventListener('mouseleave', () => {
                this.animateStatNumber(card, false);
            });
        });
    ...
    animateStatNumber(card, isHover) {
        const statNumber = card.querySelector('.stat-number');
        
        if (isHover) {
            statNumber.style.transform = 'scale(1.1)';
            statNumber.style.color = '#39ff14';
        } else {
            statNumber.style.transform = 'scale(1)';
            statNumber.style.color = '#00bfff';
        }
    }
